...
static void test_rom_bank0_read(void) {
    ...
    ASSERT_EQ(0x31, mmu_read_byte(&bus, 0x0000), "ROM bank 0 addr 0x0000");
    ASSERT_EQ(0xAA, mmu_read_byte(&bus, 0x0150), "ROM bank 0 addr 0x0150");
}
...
static void test_rom_bank1_read(void) {
    ...
    ASSERT_EQ(0xBB, mmu_read_byte(&bus, 0x4000), "ROM bank 1 start");
    ASSERT_EQ(0xCC, mmu_read_byte(&bus, 0x7FFF), "ROM bank 1 end");
}
...
static void test_boot_rom_overlay(void) {
    ...
    ASSERT_EQ(0x31, mmu_read_byte(&bus, 0x0000), "Boot ROM at 0x0000");
    ASSERT_EQ(0xEE, mmu_read_byte(&bus, 0x00FF), "Boot ROM at 0x00FF");
    /* After boot ROM range, should read game ROM */
    rom_data[0x0100] = 0xDD;
    ASSERT_EQ(0xDD, mmu_read_byte(&bus, 0x0100), "Game ROM after boot ROM range");
}
...
static void test_boot_rom_disable(void) {
    ...
    cart.boot_rom_enabled = true;
    rom_data[0x00] = 0x55;
    ASSERT_EQ(0xAA, mmu_read_byte(&bus, 0x0000), "Boot ROM active");
    /* Write to 0xFF50 disables boot ROM */
    mmu_write_byte(&bus, 0xFF50, 0x01);
    ASSERT_FALSE(cart.boot_rom_enabled, "Boot ROM disabled");
    ASSERT_EQ(0x55, mmu_read_byte(&bus, 0x0000), "Game ROM after disable");
}

/* ========== VRAM ========== */
static void test_vram_read_write(void) {
    setup();
    mmu_write_byte(&bus, 0x8000, 0x42);
    mmu_write_byte(&bus, 0x9FFF, 0x99);
    ASSERT_EQ(0x42, mmu_read_byte(&bus, 0x8000), "VRAM start");
    ASSERT_EQ(0x99, mmu_read_byte(&bus, 0x9FFF), "VRAM end");
...
static void test_wram_read_write(void) {
    setup();
    mmu_write_byte(&bus, 0xC000, 0xAA);
    mmu_write_byte(&bus, 0xDFFF, 0xBB);
    ASSERT_EQ(0xAA, mmu_read_byte(&bus, 0xC000), "WRAM start");
    ASSERT_EQ(0xBB, mmu_read_byte(&bus, 0xDFFF), "WRAM end");
}

/* ========== Echo RAM ========== */
static void test_echo_ram(void) {
    setup();
    mmu_write_byte(&bus, 0xC000, 0x77);
    /* Echo RAM at 0xE000 mirrors 0xC000 */
    ASSERT_EQ(0x77, mmu_read_byte(&bus, 0xE000), "Echo RAM mirrors WRAM");
    /* Write to echo should affect WRAM */
    mmu_write_byte(&bus, 0xE005, 0x88);
    ASSERT_EQ(0x88, mmu_read_byte(&bus, 0xC005), "Echo write mirrors to WRAM");
}

/* ========== OAM ========== */
static void test_oam_read_write(void) {
    setup();
    mmu_write_byte(&bus, 0xFE00, 0x10);
    mmu_write_byte(&bus, 0xFE9F, 0x20);
    ASSERT_EQ(0x10, mmu_read_byte(&bus, 0xFE00), "OAM start");
    ASSERT_EQ(0x20, mmu_read_byte(&bus, 0xFE9F), "OAM end");
}

/* ========== Unusable region ========== */
static void test_unusable_region(void) {
    setup();
    ASSERT_EQ(0xFF, mmu_read_byte(&bus, 0xFEA0), "Unusable region returns 0xFF");
    ASSERT_EQ(0xFF, mmu_read_byte(&bus, 0xFEFF), "Unusable region end returns 0xFF");
}

/* ========== HRAM ========== */
static void test_hram_read_write(void) {
    setup();
    mmu_write_byte(&bus, 0xFF80, 0xDE);
    mmu_write_byte(&bus, 0xFFFE, 0xAD);
    ASSERT_EQ(0xDE, mmu_read_byte(&bus, 0xFF80), "HRAM start");
    ASSERT_EQ(0xAD, mmu_read_byte(&bus, 0xFFFE), "HRAM end");
}

/* ========== IE register (0xFFFF) ========== */
static void test_ie_register(void) {
    setup();
    mmu_write_byte(&bus, 0xFFFF, 0x1F);
    ASSERT_EQ(0x1F, mmu_read_byte(&bus, 0xFFFF), "IE register");
...
static void test_ly_reset(void) {
    ...
    mmu_write_byte(&bus, 0xFF44, 0x55);
...
static void test_cart_ram_disabled(void) {
    setup();
    cart.ram_enabled = false;
    ASSERT_EQ(0xFF, mmu_read_byte(&bus, 0xA000), "Cart RAM disabled returns 0xFF");
}
...
static void test_cart_ram_enabled(void) {
    ...
    ASSERT_EQ(0x42, mmu_read_byte(&bus, 0xA000), "Cart RAM start");
    ASSERT_EQ(0x99, mmu_read_byte(&bus, 0xBFFF), "Cart RAM end");
}
...
static void test_mbc1_ram_enable(void) {
    ...
    cart.type = 0x01; /* MBC1 */
    cart.ram_enabled = false;
    mmu_write_byte(&bus, 0x0000, 0x0A); /* Enable */
    ASSERT_TRUE(cart.ram_enabled, "RAM enabled by 0x0A");
    mmu_write_byte(&bus, 0x0000, 0x00); /* Disable */
...
static void test_mbc1_rom_bank(void) {
    setup();
    cart.type = 0x01; /* MBC1 */
    mmu_write_byte(&bus, 0x2000, 0x05);
    ASSERT_EQ(5, cart.rom_bank, "MBC1 ROM bank 5");
    mmu_write_byte(&bus, 0x2000, 0x00);
...
static void test_joypad_no_selection(void) {
    ...
    uint8_t val = mmu_read_byte(&bus, 0xFF00);
